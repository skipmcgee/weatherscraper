...
class cityweather(LogFormatter):
    ...
    def app_setup(self):
        ...
        try:
            ...
            # TF 51/1 Logo Image
            logo = ImageTk.PhotoImage(Image.open(resource_path('logo.png')))
            panel = tkinter.Label(weatherapp, bg='white', image=logo)
            panel.place(x=(self.w / 2) - 50, y=(self.l / 2) - 30)

            # Labels for Entry Fields
            textlabel1 = tkinter.Label(weatherapp, font=("Helvetica", 12), justify="left", bg="light gray",
                                       text="Current Location:")
            textlabel2 = tkinter.Label(weatherapp, font=("Helvetica", 12), justify="left", bg="light gray",
                                       text="Distant Location:")
            textlabel1.grid(row=0, column=0, sticky=W + E + N + S)
            textlabel2.grid(row=1, column=0, sticky=W + E + N + S)

            # Current City Search
            city1_name = tkinter.StringVar()
            city1_entry = tkinter.Entry(weatherapp, font=("Helvetica", 12), justify="left", bg="white", fg='gray',
                                        textvariable=city1_name, width=29)
            city1_entry.grid(row=0, column=1, ipadx=0, ipady=4, sticky=N + W)

            # Sat City Search
            city2_name = tkinter.StringVar()
            city2_entry = tkinter.Entry(weatherapp, font=("Helvetica", 12), justify="left", bg="white", fg='gray',
                                        textvariable=city2_name, width=29)
            city2_entry.grid(row=1, column=1, ipadx=0, ipady=4, sticky=W + N)

            if weatherapp.init_counter:
                city1_entry.insert(0, self.cur_city)
                city2_entry.insert(0, self.sat_city)

            # Day and night images loading and resizing
            weatherapp.night = ImageTk.PhotoImage(Image.open(resource_path('night.png')))
            weatherapp.day = ImageTk.PhotoImage(Image.open(resource_path('day.png')))
            ...
            def cur_city_info():
                ...
                if cur_api_request.status_code != 200:
                    messagebox.showwarning("Weather App Warning",
                ...
                cur_offset_matches = [tz.zone for tz in map(pytz.timezone, pytz.all_timezones_set) if
                                      cur_now.astimezone(tz).utcoffset() == cur_timeoffset]
                ...
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        weatherapp.cur_image = ImageTk.PhotoImage(Image.open(resource_path('img_notfound.jpeg')))
                except Exception as error:
                    print(f"Error occured with downloading {icon1}: {error}.")
                    weatherapp.cur_image = ImageTk.PhotoImage(Image.open(resource_path('img_notfound.jpeg')))

                # Icon Placement
                label_icon1 = tkinter.Label(weatherapp, bg='white', image=weatherapp.cur_image)
                ...
                weatherapp.cur_day_night_panel = tkinter.Label(weatherapp, bg='white', image=weatherapp.day_night1)
            ...
            def sat_city_info():
                ...
                sat_offset_matches = [tz.zone for tz in map(pytz.timezone, pytz.all_timezones_set) if
                                      sat_now.astimezone(tz).utcoffset() == sat_timeoffset]
                ...
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        weatherapp.sat_image = ImageTk.PhotoImage(Image.open(resource_path('img_notfound.jpeg')))
                except Exception as error:
                    print(f"Error occured with downloading {icon2}: {error}.")
                    weatherapp.sat_image = ImageTk.PhotoImage(Image.open(resource_path('img_notfound.jpeg')))

                # Icon Placement
                label_icon2 = tkinter.Label(weatherapp, bg='white', image=weatherapp.sat_image)
                ...
                # Theme for the respective time the application is used
                weatherapp.sat_day_night_panel = tkinter.Label(weatherapp, bg='white', image=weatherapp.day_night2)
            ...
            # Country Name Labels
            label_citi1 = tkinter.Label(weatherapp, width=0, bg='white', font=("bold", 14))
            label_citi1.place(x=3, y=80)
            label_citi2 = tkinter.Label(weatherapp, width=0, bg='white', font=("bold", 14))
            label_citi2.place(x=((self.w / 2) + 3), y=80)

            # Latitude Labels
            label_lat1 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_lat1.place(x=3, y=110)
            label_lat2 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_lat2.place(x=((self.w / 2) + 3), y=110)

            # Longitude Labels
            label_lon1 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_lon1.place(x=3, y=140)
            label_lon2 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_lon2.place(x=((self.w / 2) + 3), y=140)

            # Date Labels
            label_date1 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_date1.place(x=3, y=200)
            label_date2 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_date2.place(x=((self.w / 2) + 3), y=200)

            # Time Labels
            label_time1 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_time1.place(x=3, y=170)
            label_time2 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_time2.place(x=((self.w / 2) + 3), y=170)

            # Weather Description Labels
            label_descrip1 = tkinter.Label(weatherapp, text="ERROR", width=0, bg='white', font=("Helvetica", 14))
            label_descrip1.place(x=3, y=400)
            label_descrip2 = tkinter.Label(weatherapp, text="ERROR", width=0, bg='white', font=("Helvetica", 14))
            label_descrip2.place(x=((self.w / 2) + 3), y=400)

            # Current Temperature Labels
            label_temp1 = tkinter.Label(weatherapp, text="ERROR", width=0, bg='white', font=("Helvetica", 14))
            label_temp1.place(x=3, y=430)
            label_temp2 = tkinter.Label(weatherapp, text="ERROR", width=0, bg='white', font=("Helvetica", 14))
            label_temp2.place(x=((self.w / 2) + 3), y=430)

            # Humidity Labels
            label_humidity1 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_humidity1.place(x=3, y=460)
            label_humidity2 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            label_humidity2.place(x=((self.w / 2) + 3), y=460)

            # Max Temperature Labels
            max_temp1 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            max_temp1.place(x=3, y=490)
            max_temp2 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            max_temp2.place(x=((self.w / 2) + 3), y=490)

            # Min Temperature Labels
            min_temp1 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            min_temp1.place(x=3, y=520)
            min_temp2 = tkinter.Label(weatherapp, width=0, bg='white', font=("Helvetica", 14))
            min_temp2.place(x=((self.w / 2) + 3), y=520)

            # Note at the bottom of the application label and definition
            note = tkinter.Label(weatherapp, text="All temperatures in degrees Fahrenheit", bg='white',
                                 font=("italic", 10))
            ...
            # Search Button
            city_nameButton1 = tkinter.Button(weatherapp, font=("Helvetica", 12), text="Search", command=cur_city_info)
            city_nameButton1.grid(row=0, column=2, padx=0, pady=0, sticky=W + E + N + S)
            city_nameButton2 = tkinter.Button(weatherapp, font=("Helvetica", 12), text="Search", command=sat_city_info)
